...
# ─── VERSION / TUNABLES ────────────────────────────────────
VERSION = "v6.1"            # bumped: looser filters, explicit reject logging
MEAN_WINDOW = 33
MA_WINDOW = 325
THRESHOLD = 1.5             # mean-reversion ATR-distance threshold (kept)
BREAKOUT_THRESHOLD = 1.2    # NEW: lower threshold for breakout stretched-bars (was 1.5)
BREAKOUT_BARS = 8
BREAKOUT_MIN_STRETCHED = 3  # CHANGED: 3 of 8 bars (was 5 of 8) — catches earlier
REGIME_RATIO = 0.85         # NEW: allow ATR >= 0.85 * ATR_50avg (was strictly >)
SIGNAL_EXPIRY = 14400       # 4H (trade expiry)
COOLDOWN = 7200             # CHANGED: 2H (was 4H) — allow re-entry after stop
...
WATCHLIST = [
    ...
    "TRUMP/USDT:USDT", "ENJ/USDT:USDT", "MATIC/USDT:USDT", "CYS/USDT:USDT", "ZAMA/USDT:USDT"
]
...
def handle_panic_mode(fg_score):
    if fg_score <= FG_PANIC:
        print(f"PANIC MODE — F&G: {fg_score}")
        send_telegram(
            f"🚨 <b>PANIC MODE [{VERSION}]</b>\n\n"
            f"F&G: {fg_score} — Extreme Fear\n"
            f"⚠️ New LONGs BLOCKED\n"
            f"💡 Move SLs to breakeven\n"
...
def calculate_indicators(data):
    ...
    data["tr"] = data[["High","Low","prev_close"]].apply(
        lambda r: max(r["High"]-r["Low"],
                      ...
                      abs(r["prev_close"]-r["Low"])), axis=1
    )
    data["atr_14"] = data["tr"].ewm(alpha=1/14, adjust=False).mean()
    data["atr_14"] = data["atr_14"].replace(0, 1e-8)
    ...
    data["bb_lower"]  = lower
    # NEW: Bollinger bandwidth for squeeze detection (compression → breakout)
    data["bb_width"]      = (upper - lower) / middle
    data["bb_width_avg"]  = pd.Series(data["bb_width"]).rolling(100).mean()
    data["bb_squeeze"]    = data["bb_width"] < data["bb_width_avg"] * 0.7
    ...
    # CHANGED: soft regime — ratio instead of strict >
    data["regime_ratio"] = data["atr_14"] / data["atr_avg"]
    data["is_trending"]  = data["regime_ratio"] >= REGIME_RATIO
...
def check_breakout(data_4h):
    """CHANGED: use BREAKOUT_THRESHOLD (1.2) and BREAKOUT_MIN_STRETCHED (3)."""
    recent = data_4h.tail(BREAKOUT_BARS)
    bars_stretched = (recent["atr_distance"] > BREAKOUT_THRESHOLD).sum()
    trending_up = recent["Close"].iloc[-1] > recent["Close"].iloc[0]
    last_green  = recent["Close"].iloc[-1] > recent["Close"].iloc[-2]
    return bars_stretched >= BREAKOUT_MIN_STRETCHED and trending_up and last_green

def check_squeeze_breakout(data_4h):
    """NEW: squeeze-release breakout — compression followed by expansion.
    Fires when recent bars were in a BB squeeze AND the latest bar breaks out with volume."""
    recent = data_4h.tail(10)
    had_squeeze = recent["bb_squeeze"].iloc[:-1].sum() >= 3   # at least 3 of prior 9 bars squeezed
    last = recent.iloc[-1]
    broke_out = last["Close"] > last["bb_upper"]
    vol_conf  = last["vol_spike"]
    return had_squeeze and broke_out and vol_conf

def check_1h_pullback_ok(data_1h, htf_trend):
    """NEW: allow 4H/1H mismatch if 1H is a healthy pullback in HTF direction."""
    s = data_1h.iloc[-1]
    if htf_trend == "UP":
        return s["rsi"] > 40 and s["Close"] > s["ema_slow"]
    else:
        return s["rsi"] < 60 and s["Close"] < s["ema_slow"]
    ...
    for symbol, trade in list(active_trades.items()):
        try:
            ...
            if at_breakeven:
                ...
                save_trades()
                send_telegram(
                    f"💡 <b>BREAKEVEN [{VERSION}]</b>\n\n"
                    f"<b>{symbol.replace(':USDT','')}</b>\n"
                    f"Price: {format_price(price)}\n"
                    f"➡️ Move SL to {format_price(entry)}\n"
                    f"🔒 Zero loss from here\n"
                    f"🔔 AfriMetrics {VERSION}"
                )
            if hit_sl:   closed.append((symbol,"LOSS",price))
            elif hit_tp: closed.append((symbol,"WIN",price))
            elif time.time()-trade["time"] > SIGNAL_EXPIRY:
...
def send_performance_report():
    ...
    if not stats:
        send_telegram(f"📊 <b>Performance [{VERSION}]</b>\n\nNo closed trades yet."); return
    send_telegram(
...
def check_signal(symbol, fg_score, fg_label):
    ...
    try:
        ...
        if hour_utc in DEAD_HOURS:
            print(f" SKIP — dead zone (UTC {hour_utc}:00)")
            return None
        ...
        if symbol in active_trades:
            print(f" SKIP — active trade open")
            return None
        ...
        if not has_enough_data(data_4h,10,"4H"): return None
        ...
        if not has_enough_data(data_1h,10,"1H"): return None

        data_15m = fetch_recent_data(symbol,"15m",150)
        if not has_enough_data(data_15m,3,"15m"): return None
        ...
        print(f" 4H/1H: {trend_4h}/{trend_1h} | Dist: {dist} | RSI: {rsi:.1f} | "
              f"CCI: {cci:.0f} | OBV: {'↑' if obv_up else '↓'} | "
              f"VWAP: {'✅' if above_vwap else '❌'} | Vol: {'🔥' if vol_spike else '—'} | "
              f"Regime: {regime_ratio:.2f}x {'📈' if is_trending else ('🤐' if bb_squeeze else '〰️')}")

        # ── FILTER 1: regime (SOFT) ──
        # Allow through if trending OR in a squeeze (squeeze precedes expansion)
        if not is_trending and not bb_squeeze:
            print(f" SKIP — choppy & no squeeze (ratio {regime_ratio:.2f} < {REGIME_RATIO})")
            return None

        # ── FILTER 2: HTF alignment (RELAXED) ──
        htf_mismatch_ok = False
        if trend_4h != trend_1h:
            if check_1h_pullback_ok(data_1h, trend_4h):
                htf_mismatch_ok = True
                print(f" NOTE — 1H pullback in 4H direction allowed")
            else:
                print(f" SKIP — 4H/1H mismatch (1H not in pullback)")
                return None
        ...
        squeeze_breakout = check_squeeze_breakout(data_4h)

        # ── FILTER 3: core indicator gates ──
        rsi_ok_long  = rsi < 65
        rsi_ok_short = rsi > 35
        macd_bull = macd_hist > 0
        macd_bear = macd_hist < 0
        # CHANGED: OR instead of AND (obv OR vol_spike)
        long_vol_ok = obv_up or vol_spike

        # ── SENTIMENT BLOCK ──
        if fg_score > FG_EXTREME_GREED and sell_setup:
            print(f" BLOCKED — Extreme Greed + sell setup")
            return None

        reject_reasons = []

        # ── STRATEGY 1: MEAN REVERSION LONG ──
        if buy_setup:
            if turn_up and rsi_ok_long and long_vol_ok and cci_recovering:
                ...
                active_trades[symbol] = {"entry":entry_price,"sl":sl,"tp":tp,
                    "direction":"LONG","setup":"MEAN_REV","time":time.time(),"breakeven_set":False}
                last_signal_time[symbol] = time.time()
                save_trades()
                return "BUY_REV"
            else:
                if not turn_up:          reject_reasons.append("15m not up")
                if not rsi_ok_long:      reject_reasons.append(f"RSI {rsi:.0f}>=65")
                if not long_vol_ok:      reject_reasons.append("no OBV↑ & no vol spike")
                if not cci_recovering:   reject_reasons.append(f"CCI {cci:.0f}<=-100")
        ...
        if sell_setup and turn_down and rsi_ok_short and macd_bear \
            ...
            active_trades[symbol] = {"entry":entry_price,"sl":sl,"tp":tp,
                "direction":"SHORT","setup":"MEAN_REV","time":time.time(),"breakeven_set":False}
            last_signal_time[symbol] = time.time()
            save_trades()
            return "SELL_REV"

        # ── STRATEGY 3: BREAKOUT LONG (classic) ──
        if breakout_buy and trend_4h == "UP":
            if turn_up and rsi_ok_long and macd_bull and long_vol_ok and not cci_overbought:
                entry_price = price*1.001
                ...
                active_trades[symbol] = {"entry":entry_price,"sl":sl,"tp":tp,
                    "direction":"LONG","setup":"BREAKOUT","time":time.time(),"breakeven_set":False}
                last_signal_time[symbol] = time.time()
                save_trades()
                return "BUY_BO"
            else:
                if not turn_up:        reject_reasons.append("BO: 15m not up")
                if not rsi_ok_long:    reject_reasons.append(f"BO: RSI {rsi:.0f}>=65")
                if not macd_bull:      reject_reasons.append("BO: MACD bear")
                if not long_vol_ok:    reject_reasons.append("BO: no vol/OBV")
                if cci_overbought:     reject_reasons.append(f"BO: CCI {cci:.0f}>100")

        # ── STRATEGY 4: SQUEEZE BREAKOUT LONG (NEW) ──
        if squeeze_breakout and trend_4h == "UP" and rsi_ok_long and not cci_overbought:
            entry_price = price*1.001
            sl = entry_price - atr*1.8       # tighter SL — squeeze entries are precise
            tp = entry_price + atr*3.6       # 1:2 R:R maintained
            risk = entry_price - sl
            pos_size = calculate_position_size(entry_price, sl)
            breakeven = format_price(entry_price + risk)
            send_telegram(format_signal_message(
                symbol,"LONG","SQUEEZE BREAKOUT",dist,
                format_price(entry_price),format_price(sl),format_price(tp),
                format_price(risk),format_price(risk*2),
                rsi,macd_hist,cci,fg_score,fg_label,
                breakeven,vol_spike,pos_size,above_vwap,hour_utc
            ))
            log_signal(symbol,"SQUEEZE_BO","LONG",entry_price,sl,tp,fg_score,pos_size)
            active_trades[symbol] = {"entry":entry_price,"sl":sl,"tp":tp,
                "direction":"LONG","setup":"SQUEEZE_BO","time":time.time(),"breakeven_set":False}
            last_signal_time[symbol] = time.time()
            save_trades()
            return "BUY_SQBO"

        # ── NO SIGNAL — explicit reject logging ──
        stretched = (data_4h["atr_distance"].tail(BREAKOUT_BARS) > BREAKOUT_THRESHOLD).sum()
        if reject_reasons:
            print(f" No signal | BO: {stretched}/{BREAKOUT_BARS} | CCI: {cci:.0f} | Rejected: {', '.join(reject_reasons)}")
        else:
            print(f" No signal | BO: {stretched}/{BREAKOUT_BARS} | CCI: {cci:.0f} | No setup triggered")
        return None
...
print(f" Filters: ATR+RSI+MACD+OBV+CCI+VWAP+HA+F&G+Regime+1H+Squeeze")
print(f" Regime ratio: {REGIME_RATIO} | Breakout: {BREAKOUT_MIN_STRETCHED}/{BREAKOUT_BARS} @ {BREAKOUT_THRESHOLD}")
...
send_telegram(
    ...
    f"👀 {len(WATCHLIST)} pairs\n"
    f"🔬 ATR+RSI+MACD+OBV+CCI+VWAP+HA+F&G+Squeeze\n"
...
while True:
    ...
    if scan_count % 4 == 0:
        ...
        print(f" Fear & Greed: {fg_score} — {fg_label}")
        send_telegram(
            f"💓 <b>Heartbeat [{VERSION}]</b>\n\n"
            f"👀 {len(WATCHLIST)} pairs | Active: {len(active_trades)}\n"
            f"😱 F&G: {fg_score} ({fg_label})\n"
