...
CLUSTER_BARS = 9
FORWARD_BARS = 33
THRESHOLD    = 0.5    # just above mean is enough for trend following
SLIPPAGE     = 50.0
...
ohlcv = fetch_full_history("BTC/USDT", timeframe=TIMEFRAME)
...
# ── 3. GENERATE SIGNALS ────────────────────────────────────
# TREND FOLLOWING — buy when price crosses ABOVE mean
# Opposite of mean reversion
# This is what catches the $65k → $100k type moves
THRESHOLD = 0.5
SLIPPAGE  = 50.0

data["signal"] = 0
data.loc[
    (data["atr_distance"] > THRESHOLD) &    # price above mean
    (data["trend"] == "UP") &               # long term uptrend
    (data["Close"] > data["Open"]),         # green candle confirmation
    "signal"
] = 1

print(f"\nStrategy: Trend Following | Threshold: +{THRESHOLD} ATR | Slippage: ${SLIPPAGE}")
print("Entry method: Next bar open")
...
print("\n--- BTC/USDT 4H Trend Following Backtest ---")
...
if expectancy > 0:
    print("✅ Trend Following has POSITIVE edge on BTC 4H")
else:
    print("⚠️  Trend Following has NEGATIVE edge — needs refinement")

# ── 8. COMPARISON TABLE ────────────────────────────────────
print(f"\n--- Strategy Comparison ---")
print(f"{'Strategy':<30} {'Expectancy':<15} {'Win Rate'}")
print(f"{'-'*55}")
print(f"{'Mean Reversion BTC 4H':<30} {'$-268/trade':<15} {'28.6%'}")
print(f"{'Mean Reversion ETH 4H':<30} {'$-95/trade':<15}  {'20.0%'}")
print(f"{'Trend Following BTC 4H':<30} ${expectancy:.2f}/trade{'':<5} {win_rate:.1f}%")

# ── 9. SAVE RESULTS ────────────────────────────────────────
con = duckdb.connect("analysis.db")
con.execute("DROP TABLE IF EXISTS trend_following_backtest")
con.execute("CREATE TABLE trend_following_backtest AS SELECT * FROM results_df")
con.close()
print("\nSaved to analysis.db → trend_following_backtest")
...
fig.suptitle(
    "BTC/USDT 4H — Trend Following\nBuy Above Mean in Uptrend",
    fontsize=13
)
...
plt.tight_layout()
plt.savefig("trend_following_equity_curve.png", dpi=150, bbox_inches="tight")
plt.show()
print("\nChart saved as trend_following_equity_curve.png")